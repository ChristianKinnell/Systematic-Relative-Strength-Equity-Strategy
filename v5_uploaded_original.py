...
ret_1m       = equity.iloc[-1] / equity.iloc[-min(21, len(equity)):].iloc[0] - 1
...
for _n in WF_TOPN:
    for _vc, _mask in WF_VOLCEIL_MASKS.items():
        for _atr in WF_ATR:
            ...
            _gross_ret  = _eq_series.pct_change().fillna(0)
            _cost_ret   = compute_transaction_costs(_w_c).reindex(returns.index).fillna(0)
            wf_combo_cache[(_n, _vc, _atr)] = (_gross_ret - _cost_ret)
